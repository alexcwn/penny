...
// ParseN2OSTraceLogs parses all n2os_trace.log files (including rotated logs)
func ParseN2OSTraceLogs(baseDir string, data *models.ArchiveData) error {
	logDir := resolveN2OSLogDir(baseDir)

	allEntries := collectRotatedLogs(logDir, "n2os_trace.log", 5, parseN2OSTraceLogFile)
	...
	data.N2OSTraceLogs = allEntries

	return nil
}

func parseN2OSTraceLogFile(path string) ([]models.N2OSTraceLogEntry, error) {
	...
	source := extractLogSource(filename, "n2os_trace.log", "trace")

	var entries []models.N2OSTraceLogEntry
	scanner := bufio.NewScanner(file)
	scanner.Buffer(make([]byte, 1024*1024), 1024*1024)

	timestampRegex := regexp.MustCompile(`^\[([^\]]+)\]`)
	headerRegex := regexp.MustCompile(`n2os_trace\[(\d+)\]\[(\d+)\]\s+([A-Z]+):(.*)`)
	...
	for scanner.Scan() {
		...
		if len(timestampMatch) > 1 {
			...
			entry := models.N2OSTraceLogEntry{
				RawLine:    line,
				Source:     source,
